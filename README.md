...
The supplementary material is organized as a reader-oriented guide to the additional analyses supporting CHRONOS. Each section reports either methodological details, empirical results, robustness diagnostics, or baseline comparisons useful for interpreting the main findings.

| Section                       | Title                                                 | What it contains                                                                                                                                                                  | Main takeaway                                                                                                                                                                                                                                                                                                 |
| ----------------------------- | ----------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **S1**                        | Data Processing and ICD-9 Macro-Category Mapping      | Describes how raw ICD-9-CM diagnosis codes are aggregated into CHRONOS macro-categories.                                                                                          | The aggregation reduces sparsity in the raw diagnosis-code space and makes large-scale edge-level target-trial emulation feasible, while categories such as `SUPP` remain more heterogeneous and should be interpreted cautiously.                                                                            |
| **S2**                        | Cohort Summary and Descriptive Statistics             | Reports cohort-level statistics, including number of patients, diagnosis events, calendar span, follow-up duration, age, sex distribution, and most represented macro-categories. | The processed cohort is large and longitudinally rich, with 64,828 patients, more than 9 million diagnosis events, and a median follow-up of approximately six years, supporting progression-oriented analyses.                                                                                               |
| **S3**                        | Causal Identification Assumptions                     | Formalizes the assumptions required for causal interpretation, including SUTVA, consistency, no interference, ignorability, and positivity.                                       | CHRONOS estimates can be interpreted causally only under standard observational identification assumptions; residual confounding may remain because some clinical factors are not directly observed.                                                                                                          |
| **S4**                        | Complete `D_a -> D_b` Edge Results                    | Provides the full directed disease-pair screening table, including ATE estimates, confidence intervals, placebo diagnostics, sample support, and FDR correction.                  | The full table allows readers to identify which directed disease-pair relations show stronger progression-risk signals, while also checking uncertainty, placebo behavior, sample support, and multiple-testing-adjusted significance.                                                                        |
| **S5**                        | Additional Robustness Diagnostics                     | Reports overlap, effective sample size, and placebo diagnostics across tested edges.                                                                                              | Edges with better overlap, larger effective sample size, and smaller placebo effects are more stable and more interpretable; edges with large placebo activity should be treated more cautiously even when statistically significant.                                                                         |
| **S6**                        | Implementation and Hyperparameter Configuration       | Summarizes model architecture, optimization settings, loss weights, contrastive parameters, and target-trial design choices.                                                      | The implemented CHRONOS configuration uses a compact latent representation, cross-fitting, propensity clipping, contrastive regularization, mutual-information balancing, and fixed trial-design windows to support stable edge-level estimation.                                                             |
| **S7**                        | Ablation Study                                        | Compares the full CHRONOS model with variants without contrastive regularization, without mutual-information balancing, and with predictive objectives only.                      | The full CHRONOS model achieves the strongest overall signal across representative edges, with mean ATE 0.056 and median ESS 36,465.8. Removing contrastive regularization reduces the mean ATE, while the predictive-only variant shows a much lower median ESS, suggesting weaker treated-control support.  |
| **S8**                        | Sensitivity Analysis of Trial-Design Choices          | Tests alternative buffer periods, follow-up horizons, and treated-to-control sampling ratios.                                                                                     | The results are relatively stable under moderate changes to buffer and follow-up windows, but the control-sampling ratio has a stronger impact: moving from 1:3 to 1:1 reduces ESS and removes FDR-significant findings in the representative subset.                                                         |
| **S9**                        | Comparison with Simpler Baselines                     | Compares CHRONOS with naive risk difference, logistic AIPW, and predictive-only neural baselines.                                                                                 | CHRONOS retains all representative edges after FDR correction, achieves the largest mean ATE among the compared methods, and provides substantially higher ESS than the predictive-only neural baseline, suggesting that its structured regularization improves estimation stability.                         |
| **S10**                       | Epidemiological Baseline via Edge-Specific Cox Models | Provides a complementary Cox proportional hazards analysis and compares Cox hazard ratios with CHRONOS ATE estimates.                                                             | Cox models provide an interpretable survival-analysis reference but are more fragile at scale: out of 225 directed trajectories, 168 produced valid Cox fits, 10 were FDR-significant, and only 4 remained both stable and FDR-significant after support filtering.                                           |
| **S10 additional comparison** | CHRONOS--Cox Directional Agreement                    | Compares the direction of selected CHRONOS ATE estimates with Cox hazard ratios for stable representative edges.                                                                  | Agreement is only partial, which is expected because CHRONOS estimates absolute risk differences whereas Cox models estimate relative event timing. Concordant edges increase confidence, while discordant edges indicate sensitivity to modelling assumptions or to the distinction between risk and timing. |
